...
for j = 2:dat_len - 1
	...
	for m = 2:div_n -1 
        ...
        %alpha parameter
        alpha(j,m) = -4 * pi() / lam_sig * del_index(j,m) / del_g_sig(j,m);      %usually alpha ＝ from 4 to 6
        ...
        phase_noise_change_carrier(j,m) = alpha(j,m) * gam * group_velocity * differential_gain(j,m) * del_carrier_density(j,m) / 2 * del_t; %単位的にこちら
	end
	...
	for x = 1:div_n-1
        %角度変化だけならば、angle(A_sig(j,x))だけでよい。
